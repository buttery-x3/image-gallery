...
const scriptSources = ["'self'", ...config.supportScriptOrigins].join(" ");
...
app.use((_request, response, next) => {
  ...
  response.setHeader("Content-Security-Policy", `default-src 'self'; img-src 'self' data:; script-src ${scriptSources}; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self'`);
  next();
});
    ...
    const payload: GalleryIndexResponse = {
      images: cached.images.map((image) => ({
        ...
        searchText: [
          ...
          image.shortName?.en,
          image.shortName?.ja,
          image.metadataDisplay?.name,
          image.metadataDisplay?.subtitle,
          image.batch,
          image.metadata?.schema,
          image.metadata?.resolvedPrompt,
          ...Object.values(image.metadata?.tags ?? {}),
          ...Object.values(image.metadata?.searchTokens ?? {}).flat(),
          ...Object.values(image.metadata?.facets ?? {}).flat(),
        ].filter(Boolean).join("\n").normalize("NFKC").toLocaleLowerCase(),
        tags: {
          ...(image.batch ? { batch: image.batch } : {}),
          ...(image.metadata?.tags ?? {}),
        },
        ...(Object.keys(image.metadata?.facets ?? {}).length > 0 ? { facets: image.metadata!.facets } : {}),
        ...(image.metadataDisplay ? { metadataDisplay: image.metadataDisplay } : {}),
